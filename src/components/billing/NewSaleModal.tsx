...
import { Loader2, FileText, DollarSign, Plus, Calendar, Settings2, Repeat, CreditCard, Banknote } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { useFinancialClients, useCreateClient, useFinancialCategories, useCreateTransaction, useFinancialServices, formatBRL } from '@/hooks/useFinancial';
...
function QuickSaleForm({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    if (form.payment_mode === 'parcelado' && form.installments > 1) {
      ...
      const parentNote = `${form.installments}x ${form.installment_frequency} de ${formatBRL(installmentValue)}`;
  ...
  return (
    <>
      ...
      <div className="space-y-3 mt-2">
        ...
        {showNewClient ? (
          <Input placeholder="Nome do novo cliente..." value={newClientName} onChange={e => setNewClientName(e.target.value)} className="h-9" />
        ) : (
        ...
        <div>
          ...
          <div className="flex gap-1 mt-1">
            {[
              { id: 'avista', label: '$ À Vista', icon: Banknote },
              { id: 'parcelado', label: '⊕ Parcelado', icon: CreditCard },
                ...
                className={cn(
                  ...
                  form.payment_mode === opt.id
                    ? opt.id === 'avista' ? 'bg-emerald-500 text-white border-emerald-500'
                      : opt.id === 'parcelado' ? 'bg-pink-500 text-white border-pink-500'
          ...
          <Card className="border-pink-500/20 bg-muted/30">
            <CardContent className="p-4 space-y-3">
              <p className="text-xs text-pink-400 flex items-center gap-1">
                ...
                <strong>Parcelado:</strong> Divide o valor total em parcelas fixas com datas definidas. Ideal para vendas com recebimento parcelado.
              </p>
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground mt-0.5">Ajuste para ajustar — o sistema calcula automaticamente</p>
              </div>
          ...
          <Card className="border-pink-500/20 bg-muted/30">
            <CardContent className="p-4 space-y-3">
              <p className="text-xs text-pink-400 flex items-center gap-1">
                ...
                <strong>Recorrente:</strong> Cria uma assinatura com cobrança automática no ciclo escolhido. Ideal para mensalidades, planos e serviços contínuos.
              </p>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <button
                    onClick={() => setForm(f => ({ ...f, recurring_duration: 'until_cancel' }))}
                    className={cn(
                      'w-full text-left p-3 rounded-lg border transition-colors',
                      form.recurring_duration === 'until_cancel' ? 'border-pink-500 bg-pink-500/10' : 'border-border hover:bg-accent'
                    )}>
                    ...
                    <p className="text-[10px] text-muted-foreground ml-5">Cobra automaticamente a cada ciclo até que você ou o cliente cancele a assinatura manualmente.</p>
                  </button>
                  <button
                    onClick={() => setForm(f => ({ ...f, recurring_duration: 'fixed' }))}
                    className={cn(
                      'w-full text-left p-3 rounded-lg border transition-colors',
                      form.recurring_duration === 'fixed' ? 'border-pink-500 bg-pink-500/10' : 'border-border hover:bg-accent'
                    )}>
                    <p className="text-sm font-semibold flex items-center gap-1">
                    ...
                    <p className="text-[10px] text-muted-foreground ml-5">Define um número exato de cobranças. A assinatura encerra automaticamente ao atingir o limite.</p>
                  </button>
                </div>
