...
import { ChevronLeft, ChevronRight, Clock, Flag, User, Building2, X, Calendar, Tag, DollarSign, CheckSquare, Plus, Trash2, FileText, Users, AlertTriangle } from 'lucide-react';
...
function DealSidePanel({ deal, columns, onClose }: { deal: KanbanDeal; columns: KanbanColumn[]; onClose: () => void }) {
  ...
  return (
    <div className="w-[380px] flex-shrink-0 border-l bg-card overflow-y-auto animate-fade-in">
      {/* Header */}
...
function InfoRow({ icon, label, value, muted }: { icon: React.ReactNode; label: string; value: React.ReactNode; muted?: boolean }) {
  return (
    <div className="flex items-start gap-2 text-sm">
      ...
      <span className={cn('flex-1', muted && 'text-muted-foreground')}>{value}</span>
    </div>
  );
}
...
export function CalendarView({ deals, columns, onDetail }: CalendarViewProps) {
  ...
  return (
    <div className="max-w-[1800px] mx-auto px-3 sm:px-4 py-4 flex gap-0">
                      ...
                      return (
                        ...
                        >
                          <span className="flex items-center gap-1">
                            ...
                            {isOverdue && !isSelected && <Clock className="w-2.5 h-2.5 flex-shrink-0" />}
                          </span>
                        </button>
                      );
                    })}
